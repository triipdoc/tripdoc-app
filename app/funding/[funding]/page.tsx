...
function formatFundingName(value: string) {
...
export async function generateMetadata({
  ...
  const title = `${formattedFunding} Opportunities | TripDoc`;
  const description = `Browse verified ${formattedFunding.toLowerCase()} opportunities on TripDoc, including scholarships, internships, fellowships, jobs, and more.`;

  return {
    title,
    description,
    alternates: {
      canonical: `${SITE_URL}/funding/${funding}`,
    },
    openGraph: {
      title,
      description,
      url: `${SITE_URL}/funding/${funding}`,
      siteName: "TripDoc",
      type: "website",
    },
    twitter: {
...
export default async function FundingPage({
  ...
  return (
    <main style={{ maxWidth: 1200, margin: "0 auto", padding: "24px 20px 40px" }}>
      ...
      >
        {formattedFunding} Opportunities
      </h1>

      <p style={{ marginTop: 10, color: "#666", lineHeight: 1.7 }}>
        Explore verified {formattedFunding.toLowerCase()} opportunities on TripDoc.
      </p>
      ...
      {programs.length === 0 ? (
        ...
        >
          ...
          <p style={{ margin: 0, color: "#666", lineHeight: 1.7 }}>
            There are no verified {formattedFunding.toLowerCase()} opportunities yet.
