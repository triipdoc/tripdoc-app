...
export async function generateMetadata({
  ...
  const title = `Opportunities in ${formattedCountry} | TripDoc`;
  const description = `Browse verified scholarships, internships, fellowships, and global opportunities in ${formattedCountry} on TripDoc.`;

  return {
    title,
    description,
    alternates: {
      canonical: `${SITE_URL}/countries/${country}`,
    },
    openGraph: {
      title,
      description,
...
export default async function CountryPage({
  ...
  return (
    <main style={{ maxWidth: 1200, margin: "0 auto", padding: "24px 20px 40px" }}>
      ...
      >
        Opportunities in {formattedCountry}
      </h1>

      <p style={{ marginTop: 10, color: "#666", lineHeight: 1.7 }}>
        Explore verified scholarships, internships, fellowships, jobs, and more in{" "}
        {formattedCountry}.
      </p>
      ...
      >
        {programs.length} opportunity{programs.length === 1 ? "" : "ies"} found
      </div>
      ...
      {programs.length === 0 ? (
        ...
        >
          ...
          <p style={{ margin: 0, color: "#666", lineHeight: 1.7 }}>
            There are no verified opportunities for {formattedCountry} yet. Check back
            later or browse all available opportunities on TripDoc.
          </p>
        </div>
      ) : (
        <div
